...
harmful for some [Rothwell, Lancet 1995; ...].
...
```{r power, cache=TRUE, echo=FALSE, fig.cap="Statistical power for the detection of an interaction when the interaction effect size is between 0 and four times the main effect size. For simplicity, we assume equal number of treated and untreated patients and that patients are equally separated between the subgroup levels", fig.show="hold", out.width = '100%'}
...
1. Explore the literature for existing predictive approaches to the assessment
...
2. Develop and apply a standardized framework for risk-stratified assessment of
...
4. Develop and compare methods for predicting personalized treatment effects
...
5. Develop statistical models that are easily applicable in clinical practice
...
In **Chapter 4** we apply the standardized framework to evaluate effect
...
In **Chapter 5** we compare different risk-based methods for predicting
...
In **Chapter 6** we develop a model for the prediction of 5-year recurrence risk
...
In **Chapter 7** we develop a model for the prediction of 28-day mortality for
...
Finally, in **Chapter 8** we present a general discussion along with perspectives on
future work.
